...
for (i in party_grid){
  ...
  if (i == "Pooled"){
    ...
    data_2020 <- droplevels(CaltechClimate_Nov2020[CaltechClimate_Nov2020$PartyID_recode == i,])
    data_2022 <- droplevels(CaltechClimate_Nov2022[CaltechClimate_Nov2022$PartyID_recode == i,])
  }
  ...
  if(i!="Republican"){
    margins_2020          <- summary(margins(regression_2020[[idx]], design = design_2020))[, c("factor",'AME', 'SE')]}else{
      margins_2020          <- summary(margins(regression_2020[[idx]], design = design_2020))[, c("factor",'AME', 'SE')]  
    }
  ...
  if(i!="Republican"){
    margins_2022         <- summary(margins(regression_2022[[idx]], design = design_2022))[, c("factor",'AME', 'SE')]}else{
      margins_2022         <- summary(margins(regression_2022[[idx]], design = design_2022))[, c("factor",'AME', 'SE')]  
    }
  
  if (idx == 1){
    margin.reduced <- rbind(data.frame("Party" = i, "Year" = 2020,margins_2020),
                            data.frame("Party" = i, "Year" = 2022,margins_2022)
                            )
    
            
  } else {
    margin.reduced <- margin.reduced %>% add_row(rbind(data.frame("Party" = i, "Year" = 2020,margins_2020),
                                                       data.frame("Party" = i, "Year" = 2022,margins_2022))
                                                       )
  }
  idx <- idx+1
...
save(margin.reduced,regression_2020,regression_2022,file="Data/counterfactual_all_variables_recode.RData")
